const parts = await Promise.all([0,1,2,3,4].map(i => fetch(new URL("./List_chunk_"+i+".txt", import.meta.url)).then(r => { if (!r.ok) throw new Error("List_chunk_"+i); return r.text(); })));
