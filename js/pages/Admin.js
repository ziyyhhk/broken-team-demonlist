...
<p class="admin-hint">Placeholders: {mention} {player} {level} {ordinal} {rank} {percent} {link} {link_line}</p>
...
<label style="display:flex;align-items:center;gap:0.4rem;margin-bottom:0.5rem"><input type="checkbox" v-model="announceOnSave" /> Send Discord posts when saving levels</label>
...
<div class="admin-actions"><button type="button" class="auth-btn" @click="saveToken">Save token</button><button type="button" class="auth-btn auth-btn--ghost" @click="testToken">Test</button></div>
  ...
  methods: {
    ...
    async saveDiscordMessages() {
      ...
      this.discordMessages = Object.assign({}, payload);
