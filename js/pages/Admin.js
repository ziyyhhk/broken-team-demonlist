...
export default {
  ...
  template: `<main class="page-admin page-shell">
...
<h2>Player ID</h2>
<p class="admin-hint">Link list names to Discord IDs for @mentions. Enable Developer Mode → right-click user → Copy User ID.</p>
<div class="admin-edit-card" style="margin-bottom:1rem">
<h3 style="margin:0 0 0.5rem">Discord webhook</h3>
<p class="admin-hint">Channel → Integrations → Webhooks → Copy URL. Stored in this browser only.</p>
...
<div class="admin-edit-card" style="margin-bottom:1rem">
<h3 style="margin:0 0 0.5rem">Announcement messages</h3>
<p class="admin-hint">Placeholders: {mention} {player} {level} {ordinal} {rank} {top} {list_rank} {old_rank} {old_ordinal} {direction} {percent} {link} {link_line}</p>
<label style="display:flex;align-items:center;gap:0.4rem;margin-bottom:0.4rem"><input type="checkbox" v-model="discordMessages.enabledVictor" /> Announce new victors (100%)</label>
<textarea class="admin-ta" rows="2" v-model="discordMessages.victor" style="width:100%;font-family:monospace;font-size:0.85rem;margin-bottom:0.5rem"></textarea>
<label style="display:flex;align-items:center;gap:0.4rem;margin-bottom:0.4rem"><input type="checkbox" v-model="discordMessages.enabledVerify" /> Announce verifications</label>
<textarea class="admin-ta" rows="2" v-model="discordMessages.verify" style="width:100%;font-family:monospace;font-size:0.85rem;margin-bottom:0.5rem"></textarea>
<label style="display:flex;align-items:center;gap:0.4rem;margin-bottom:0.4rem"><input type="checkbox" v-model="discordMessages.enabledMove" /> Announce list moves (when you Save main order)</label>
...
<p class="admin-hint" style="margin-top:-0.25rem">Move placeholders: {level} {rank} {ordinal} {old_rank} {old_ordinal} {direction}</p>
<label style="display:flex;align-items:center;gap:0.4rem;margin-bottom:0.5rem"><input type="checkbox" v-model="announceOnSave" /> Send Discord posts when saving levels / order</label>
<div class="admin-actions">
<button type="button" class="auth-btn" :disabled="saving" @click="saveDiscordMessages">Save message templates</button>
<button type="button" class="auth-btn auth-btn--ghost" @click="previewVictorMsg">Preview victor</button>
<button type="button" class="auth-btn auth-btn--ghost" @click="previewVerifyMsg">Preview verify</button>
<button type="button" class="auth-btn auth-btn--ghost" @click="previewMoveMsg">Preview move</button>
</div></div>
...
<span class="admin-order__rank">#{{ i+1 }}</span><span>{{ p }}</span>
...
<label>Creator / Author * <input class="admin-input" v-model="newImp.author" /></label>
<label>Verifier <input class="admin-input" v-model="newImp.verifier" placeholder="Leave empty if unverified" /></label>
<label class="admin-grid--full">Video (optional) <input class="admin-input" v-model="newImp.verification" /></label>
...
<button type="button" @click="promoteImpossible(impossibleOrder.indexOf(p))">→ Main</button>
</span></li></ul>
...
<input class="admin-input" v-model.number="r.percent" type="number" min="1" max="100" style="width:4.5rem" />
...
<label>Place at rank <input class="admin-input" type="number" min="1" :max="listOrder.length + 1" v-model.number="newLevel.placeAt" /></label>
</div>
<p class="admin-hint" style="margin:0.35rem 0 0">1 = top of list. e.g. 4 puts it at #4 (others shift down).</p>
...
<input class="admin-input" v-model.number="r.percent" type="number" min="1" max="100" style="width:4.5rem" />
<input class="admin-input" v-model="r.link" placeholder="Video URL" style="flex:2;min-width:10rem" />
<button type="button" class="auth-btn auth-btn--ghost" style="padding:0.2rem 0.4rem" @click="moveRecord(ri,-1)" :disabled="ri===0">↑</button>
<button type="button" class="auth-btn auth-btn--ghost" style="padding:0.2rem 0.4rem" @click="moveRecord(ri,1)" :disabled="ri===draftRecords.length-1">↓</button>
<button type="button" class="rec-del" @click="draftRecords.splice(ri,1)">✕</button>
</div>
<div class="rec-table__row">
<span class="admin-order__rank">+</span>
...
<input class="admin-input" v-model="lv.verification" placeholder="Main video URL" style="width:100%;margin-bottom:0.3rem" />
</li></ul>
</div>

<div v-if="tab==='info' && canLevels" class="admin-panel"><h2>Info</h2><textarea class="admin-ta" v-model="infoText" rows="12"></textarea><div class="admin-actions"><button type="button" class="auth-btn" :disabled="saving" @click="saveInfo">Save info</button></div></div>
<div v-if="tab==='rules' && canLevels" class="admin-panel"><h2>Rules</h2><textarea class="admin-ta" v-model="rulesText" rows="12"></textarea><div class="admin-actions"><button type="button" class="auth-btn" :disabled="saving" @click="saveRules">Save rules</button></div></div>
<div v-if="tab==='editors' && canEditors" class="admin-panel"><h2>Editors</h2><textarea class="admin-ta" v-model="editorsText" rows="10"></textarea><div class="admin-actions"><button type="button" class="auth-btn" :disabled="saving" @click="saveEditors">Save editors</button></div></div>
<div v-if="tab==='users' && canUsers" class="admin-panel"><h2>Users</h2>
<div class="admin-grid">
...
<div class="admin-actions"><button type="button" class="auth-btn" @click="saveToken">Save token</button><button type="button" class="auth-btn auth-btn--ghost" @click="testToken">Test</button></div>
  ...
  methods: {
    ...
    async openPlayers() { this.tab = 'players'; await this.reloadPlayers(); },
    ...
    setPlayerDiscord(name, id) {
      ...
      if (!clean) Object.keys(next).forEach((k) => { if (k.toLowerCase() === String(name).toLowerCase()) delete next[k]; });
      else { Object.keys(next).forEach((k) => { if (k.toLowerCase() === String(name).toLowerCase() && k !== name) delete next[k]; }); next[name] = clean; }
    ...
    addManualPlayer() {
      ...
      this.manualPlayerName = ''; this.manualPlayerId = '';
      this.flash('Added ' + name + ' — click Save Discord IDs.');
    },
    saveWebhookLocal() {
      try { localStorage.setItem(WEBHOOK_KEY, (this.discordWebhook || '').trim()); this.flash('Webhook saved in this browser.'); }
      catch (e) { this.flash('Could not save webhook locally.', true); }
    },
    async testWebhook() {
      ...
      try {
        const res = await fetch(url, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ content: 'Broken Team Demonlist webhook test.' }) });
        if (res.ok || res.status === 204) this.flash('Test message sent to Discord.');
        else this.flash('Webhook failed: HTTP ' + res.status, true);
      } catch (e) { this.flash('Webhook error: ' + (e.message || e), true); }
      finally { this.testingWebhook = false; }
    },
    async reloadPlayers() {
      try { const [lb] = await fetchLeaderboard(); this.leaderboard = Array.isArray(lb) ? lb : []; } catch (e) { this.leaderboard = []; }
      try {
        const res = await fetch('./data/_player_discord.json?t=' + Date.now(), { cache: 'no-store' });
        if (res.ok) { const data = await res.json(); this.playerDiscord = data && typeof data === 'object' && !Array.isArray(data) ? data : {}; }
        else this.playerDiscord = {};
      } catch (e) { this.playerDiscord = {}; }
      try { this.discordWebhook = localStorage.getItem(WEBHOOK_KEY) || ''; } catch (e) { this.discordWebhook = ''; }
      try {
        const res = await fetch('./data/_discord_messages.json?t=' + Date.now(), { cache: 'no-store' });
        if (res.ok) this.discordMessages = Object.assign({}, DEFAULT_MESSAGES, await res.json());
      } catch (e) {}
    },
    async savePlayerDiscord() {
      ...
      await this.pushFile('data/_player_discord.json', JSON.stringify(cleaned, null, 4), 'Admin: player Discord IDs');
    },
    async saveDiscordMessages() {
      const payload = {
        victor: this.discordMessages.victor || DEFAULT_MESSAGES.victor,
        verify: this.discordMessages.verify || DEFAULT_MESSAGES.verify,
        move: this.discordMessages.move || DEFAULT_MESSAGES.move,
        enabledVictor: !!this.discordMessages.enabledVictor,
        enabledVerify: !!this.discordMessages.enabledVerify,
        enabledMove: this.discordMessages.enabledMove !== false,
      };
      this.discordMessages = Object.assign({}, DEFAULT_MESSAGES, payload);
      await this.pushFile('data/_discord_messages.json', JSON.stringify(payload, null, 4), 'Admin: Discord message templates');
    },
    previewVictorMsg() {
      this.flash('Preview: ' + formatMessage(this.discordMessages.victor, buildVars({ player: 'ExamplePlayer', discordId: '123456789012345678', level: 'Broken Circles', rank: 5, percent: 100, link: 'https://youtu.be/example' })));
    },
    previewVerifyMsg() {
      this.flash('Preview: ' + formatMessage(this.discordMessages.verify, buildVars({ player: 'ExampleVerifier', discordId: '123456789012345678', level: 'Broken Circles', rank: 1, top: 1, list_rank: 1, link: 'https://youtu.be/example' })));
    },
    previewMoveMsg() {
      this.flash('Preview: ' + formatMessage(this.discordMessages.move || DEFAULT_MESSAGES.move, buildVars({ level: 'Broken Circles', rank: 3, old_rank: 12, direction: 'up' })));
    },
    levelNameFromPath(path) {
      const found = (this.list || []).find((pair) => pair[0] && pair[0].path === path);
      return (found && found[0] && found[0].name) || path;
    },
    async ensureDiscordData() {
      try {
        const res = await fetch('./data/_player_discord.json?t=' + Date.now(), { cache: 'no-store' });
        if (res.ok) { const data = await res.json(); if (data && typeof data === 'object' && !Array.isArray(data)) this.playerDiscord = data; }
      } catch (e) {}
      try {
        const res = await fetch('./data/_discord_messages.json?t=' + Date.now(), { cache: 'no-store' });
        if (res.ok) this.discordMessages = Object.assign({}, DEFAULT_MESSAGES, await res.json());
      } catch (e) {}
      try { this.discordWebhook = localStorage.getItem(WEBHOOK_KEY) || this.discordWebhook || ''; } catch (e) {}
    },
    async runAnnouncements(prev, next) {
      await this.ensureDiscordData();
      const url = (this.discordWebhook || '').trim();
      if (!url) return;
      const events = diffLevelAnnouncements(prev, next);
      let listTop = null;
      if (this.selectedPath && Array.isArray(this.listOrder)) {
        const i = this.listOrder.indexOf(this.selectedPath);
        if (i >= 0) listTop = i + 1;
      }
      let sent = 0;
      for (const ev of events) {
        if (ev.type === 'victor' && !this.discordMessages.enabledVictor) continue;
        if (ev.type === 'verify' && !this.discordMessages.enabledVerify) continue;
        const tpl = ev.type === 'verify' ? this.discordMessages.verify : this.discordMessages.victor;
        const rankForMsg = ev.type === 'verify' ? (ev.top != null ? ev.top : listTop) : ev.rank;
        const topForMsg = ev.type === 'verify' ? (ev.top != null ? ev.top : listTop) : listTop;
        const content = formatMessage(tpl, buildVars({
          player: ev.player,
          discordId: resolveDiscordId(this.playerDiscord, ev.player),
          level: ev.level,
          rank: rankForMsg,
          top: topForMsg,
          list_rank: topForMsg,
        ...
        const r = await sendDiscordWebhook(url, content);
        if (r.ok) sent += 1;
      }
      if (sent) this.flash('Discord: sent ' + sent + ' announcement(s).');
    },
    async runListMoveAnnouncements(prevOrder, nextOrder) {
      await this.ensureDiscordData();
      if (this.discordMessages.enabledMove === false) return;
      const url = (this.discordWebhook || '').trim();
      if (!url) return;
      const events = diffListOrder(prevOrder, nextOrder);
      if (!events.length) return;
      const tpl = this.discordMessages.move || DEFAULT_MESSAGES.move;
      let sent = 0;
      for (const ev of events) {
        const content = formatMessage(tpl, buildVars({
          level: this.levelNameFromPath(ev.path),
          rank: ev.rank,
      ...
      if (sent) this.flash('Discord: sent ' + sent + ' list-move announcement(s).');
    },
    ...
    async moveToImpossible(i) {
      const p = this.listOrder[i]; if (!p) return;
      const order = this.listOrder.slice(); order.splice(i, 1);
      const imp = this.impossibleOrder.slice(); if (!imp.includes(p)) imp.push(p);
      this.listOrder = order; this.impossibleOrder = imp;
      await this.saveList(); await this.saveImpossible(); this.flash(p + ' moved to Impossible.');
    },
    async promoteImpossible(i) {
      const p = this.impossibleOrder[i]; if (!p) return;
      const imp = this.impossibleOrder.slice(); imp.splice(i, 1);
      const order = this.listOrder.slice(); if (!order.includes(p)) order.unshift(p);
      this.impossibleOrder = imp; this.listOrder = order;
      await this.saveImpossible(); await this.saveList(); this.flash(p + ' promoted to Main #1.');
    },
    selectLevel(p) {
      this.selectedPath = p;
      const found = this.list.find((pair) => pair[0] && pair[0].path === p);
      if (!found || !found[0]) { this.draft = null; this.draftRecords = []; this.levelSnapshot = null; return; }
      ...
      this.levelSnapshot = { name: found[0].name, verifier: found[0].verifier || '', verification: found[0].verification || '', records: JSON.parse(JSON.stringify(found[0].records || [])) };
    },
    toggleDraftTag(tag) {
      if (!this.draft) return;
      if (!Array.isArray(this.draft.tags)) this.draft.tags = [];
      const i = this.draft.tags.indexOf(tag);
      if (i === -1) this.draft.tags.push(tag); else this.draft.tags.splice(i, 1);
    },
    toggleNewLevelTag(tag) {
      if (!Array.isArray(this.newLevel.tags)) this.newLevel.tags = [];
      const i = this.newLevel.tags.indexOf(tag);
      if (i === -1) this.newLevel.tags.push(tag); else this.newLevel.tags.splice(i, 1);
    },
    toggleImpTag(tag) {
      if (!Array.isArray(this.newImp.tags)) this.newImp.tags = [];
      const i = this.newImp.tags.indexOf(tag);
      if (i === -1) this.newImp.tags.push(tag); else this.newImp.tags.splice(i, 1);
    },
    moveRecord(i, dir) {
      const j = i + dir; if (j < 0 || j >= this.draftRecords.length) return;
      const a = this.draftRecords.slice(); const tmp = a[i]; a[i] = a[j]; a[j] = tmp; this.draftRecords = a;
    },
    addRecord() {
    ...
    async createImpossibleLevel() {
      ...
      const payload = { id: Number(n.id) || 0, name: n.name.trim(), author, creators: [author], verifier: (n.verifier || '').trim(), verification: (n.verification || '').trim(), thumbnail: (n.thumbnail || '').trim(), percentToQualify: 100, password: 'Free to Copy', length: n.length || '', creationDate: new Date().toLocaleDateString('en-US'), tags, records: [] };
      ...
      this.selectLevel(path); this.flash('Added to Impossible.');
    },
    async createLevel() {
      ...
      let path = slugify(n.name);
      if (this.listOrder.includes(path) || this.impossibleOrder.includes(path)) path = path + Date.now().toString().slice(-4);
      const payload = { id: Number(n.id) || 0, name: n.name.trim(), author: (n.author || n.verifier).trim(), creators: [(n.author || n.verifier).trim()], verifier: n.verifier.trim(), verification: n.verification.trim(), thumbnail: (n.thumbnail || '').trim(), percentToQualify: Number(n.percentToQualify) || 100, password: 'Free to Copy', length: n.length || '', creationDate: new Date().toLocaleDateString('en-US'), tags: Array.isArray(n.tags) ? n.tags.slice() : [], records: [] };
      ...
      let place = Number(n.placeAt);
      if (!Number.isFinite(place) || place < 1) place = 1;
      if (place > order.length + 1) place = order.length + 1;
      const insertAt = place - 1;
      order.splice(insertAt, 0, path);
      if (!(await this.pushFile('data/_list.json', JSON.stringify(order, null, 4), 'Admin: list add'))) return;
      this.listOrder = order;
      this.listOrderSnapshot = order.slice();
      this.list.splice(insertAt, 0, [Object.assign({}, payload, { path }), null]);
      this.showAddLevel = false;
      this.newLevel = { name: '', id: '', author: '', verifier: '', verification: '', thumbnail: '', length: '', percentToQualify: 100, tags: [], targetList: 'main', placeAt: 1 };
      this.selectLevel(path); this.flash('Level added at Main #' + place + '.');
      if (this.announceOnSave && payload.verifier) await this.runAnnouncements(null, payload);
    },
    async saveLevel() {
      if (!this.draft || !this.selectedPath) return;
      if (this.isSelectedImpossible && Array.isArray(this.draft.creators) && this.draft.author) this.draft.creators = [this.draft.author];
      const payload = Object.assign({}, this.draft, { records: this.draftRecords || [] });
      const prev = this.levelSnapshot;
      ...
      if (ok) {
        ...
        if (this.announceOnSave) await this.runAnnouncements(prev, payload);
        this.levelSnapshot = { name: payload.name, verifier: payload.verifier || '', verification: payload.verification || '', records: JSON.parse(JSON.stringify(payload.records || [])) };
        if (this.isSelectedImpossible) {
          const hasClear = (this.draftRecords || []).some((r) => Number(r.percent) === 100);
          if (hasClear) { const idx = this.impossibleOrder.indexOf(this.selectedPath); if (idx !== -1) await this.promoteImpossible(idx); }
        }
      }
    },
    async saveList() {
      const prev = (this.listOrderSnapshot && this.listOrderSnapshot.length) ? this.listOrderSnapshot.slice() : [];
      const next = (this.listOrder || []).slice();
      const ok = await this.pushFile('data/_list.json', JSON.stringify(next, null, 4), 'Admin: order');
      if (ok) {
        if (this.announceOnSave && prev.length) await this.runListMoveAnnouncements(prev, next);
        this.listOrderSnapshot = next.slice();
      }
    },
    async saveImpossible() { await this.pushFile('data/_impossible.json', JSON.stringify(this.impossibleOrder, null, 4), 'Admin: impossible order'); },
    async saveConfig() {
    ...
    async openServerHardest() {
      ...
      try {
        ...
        if (res.ok) {
          ...
          this.serverLevels = Array.isArray(data) ? data.map(function (l) { const o = Object.assign({}, l); o.records = Array.isArray(o.records) ? o.records : []; return o; }) : [];
        } else this.serverLevels = [];
      } catch (e) { this.serverLevels = []; }
    },
    shAddRow() { this.serverLevels.push({ id: '', name: '', author: '', verifier: '', verification: '', thumbnail: '', length: '', records: [] }); },
    ...
    async shRemove(i) { if (!confirm('Remove this level?')) return; this.serverLevels.splice(i, 1); },
    async saveServerHardest() {
      const payload = this.serverLevels.map(function (l) { const o = Object.assign({}, l); o.records = (o.records || []).filter(function (r) { return r && String(r.user || '').trim(); }); return o; });
      await this.pushFile('data/_server_hardest.json', JSON.stringify(payload, null, 4), 'Admin: Server Hardest');
    },
    async saveInfo() { try { const data = JSON.parse(this.infoText); await this.pushFile('data/info.json', JSON.stringify(data, null, 4), 'Admin: info'); } catch (e) { this.flash('Invalid JSON', true); } },
    async saveRules() { try { const data = JSON.parse(this.rulesText); await this.pushFile('data/rules.json', JSON.stringify(data, null, 4), 'Admin: rules'); } catch (e) { this.flash('Invalid JSON', true); } },
    async saveEditors() { try { const data = JSON.parse(this.editorsText); await this.pushFile('data/_editors.json', JSON.stringify(data, null, 4), 'Admin: editors'); } catch (e) { this.flash('Invalid JSON', true); } },
  ...
  async mounted() {
    ...
    this.listOrderSnapshot = this.listOrder.slice();
    const imp = (await fetchImpossible()) || [];
    this.impossibleOrder = imp.map((p) => (p[0] && p[0].path) || p[1]).filter(Boolean);
    imp.forEach((pair) => { if (pair[0] && pair[0].path && !this.list.some((x) => x[0] && x[0].path === pair[0].path)) this.list.push(pair); });
