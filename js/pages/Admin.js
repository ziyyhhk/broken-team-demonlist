export default Vue.defineAsyncComponent(async () => {
  ...
  var shMethods = [
    ...
    "          this.serverLevels = Array.isArray(data) ? data.map(function(l) { return Object.assign({}, l, { records: l.records || [] }); }) : [];",
    "        } else this.serverLevels = [];",
    "      } catch (e) { this.serverLevels = []; }",
    "    },",
    "    shAddRow() {",
    "      this.serverLevels.push({ id: '', name: '', author: '', verifier: '', verification: '', length: '', note: '', tags: [], records: [] });",
    "    },",
    "    shMove(i, dir) {",
    "      var j = i + dir;",
    ...
    "      await this.pushFile('data/_server_hardest.json', JSON.stringify(this.serverLevels, null, 4), 'Admin: Server Hardest (' + this.serverLevels.length + ' levels)');",
    "    },"
  ].join('\n');
  ...
  if (code.indexOf('async removeFromList(') === -1) {
    ...
    code = code.replace('async saveList() {', rm);
  }
  ...
  var newPanel = [
    ...
    '<h2>Server Hardest</h2>',
    '<p class="admin-hint">Like Tiers: + Add level, X removes (saves now), arrows reorder, Save all for edits.</p>',
  ...
  if (code.indexOf(oldPanel) !== -1) {
    code = code.split(oldPanel).join(newPanel);
  } else {
    console.warn('[Admin] Server Hardest panel not found');
  }
