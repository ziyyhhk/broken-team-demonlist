...
<p class="admin-hint">Placeholders: {mention} {player} {level} {ordinal} {rank} {old_rank} {old_ordinal} {direction} {percent} {link} {link_line}</p>
...
<div class="admin-actions"><button type="button" class="auth-btn" @click="saveToken">Save token</button><button type="button" class="auth-btn auth-btn--ghost" @click="testToken">Test</button></div>
  ...
  methods: {
    ...
    previewVerifyMsg() {
      this.flash('Preview: ' + formatMessage(this.discordMessages.verify, buildVars({ player: 'ExampleVerifier', discordId: '123456789012345678', level: 'Broken Circles', link: 'https://youtu.be/example' })));
    },
    previewMoveMsg() {
    ...
    async runAnnouncements(prev, next) {
      ...
      for (const ev of events) {
        ...
        const content = formatMessage(tpl, buildVars({ player: ev.player, discordId: resolveDiscordId(this.playerDiscord, ev.player), level: ev.level, rank: ev.rank, percent: ev.percent, link: ev.link }));
