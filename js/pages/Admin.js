...
export default {
    ...
    data: () => ({
        ...
        board: [], boardPlayer: null, boardRows: [], verifiedRows: [], playerLookup: '', setVerifierPath: '', activityLogs: [],
            ...
            <section class="admin-main">
                ...
                <div v-if="tab === 'levels' && canLevels" class="admin-panel admin-panel--wide">
                    ...
                    <div class="admin-howto">
                        ...
                        <ol>
                            <li><b>Verifier</b> → field “Player who verified”</li>
                            <li><b>Victor</b> → table below</li>
                            <li>Or Leaderboard tab → Set as verifier</li>
                        </ol>
                    </div>
                    ...
                    <template v-if="draft">
                        <div class="admin-edit-card">
                            ...
                            <div class="admin-grid">
                                ...
                                <label>Player who verified <input class="admin-input" v-model="draft.verifier" placeholder="Real name" /></label>
                ...
                <div v-if="tab === 'board' && canLevels" class="admin-panel admin-panel--wide">
                    ...
                    <div class="admin-howto">
                        <ol>
                            <li>Type name → <b>Open / create player</b></li>
                            <li><b>VERIFIED:</b> pick level → <b>Set as verifier</b></li>
                            <li><b>COMPLETED:</b> Add level they beat → Save</li>
                        </ol>
                    </div>
                    ...
                    <div class="board-layout">
                        ...
                        <p v-else class="admin-hint">Type a player name and click Open.</p>
                    </div>
                </div>
                ...
                <div v-if="tab === 'log' && (canLevels || canList)" class="admin-panel">
                    <h2>Activity log</h2>
                    <p class="admin-hint">Recent admin saves (data/_logs.json). Newest first.</p>
                    <div class="admin-actions">
                        <button type="button" class="auth-btn auth-btn--ghost" @click="openLog">Refresh</button>
                        <button type="button" class="auth-btn auth-btn--ghost" :disabled="saving || !activityLogs.length" @click="clearLogs">Clear log</button>
                    </div>
                    <ul class="admin-userlist" v-if="activityLogs.length">
                        <li v-for="(entry, i) in activityLogs" :key="i" style="flex-direction:column;align-items:flex-start;gap:0.2rem">
                            <div style="display:flex;gap:0.5rem;flex-wrap:wrap;width:100%">
                                <span class="admin-role-tag">{{ entry.action || 'edit' }}</span>
                                <strong>{{ entry.user || '?' }}</strong>
                                <span class="admin-muted">{{ entry.at || '' }}</span>
                            </div>
                            <span style="font-size:0.85rem">{{ entry.detail || entry.message || '' }}</span>
                        </li>
                    </ul>
                    <p v-else class="admin-hint">No entries yet.</p>
                </div>
    ...
    methods: {
        ...
        async appendLog(detail, path) {
            ...
            var logs = [];
            try {
                var res = await fetch('./data/_logs.json?t=' + Date.now(), { cache: 'no-store' });
                if (res.ok) {
        ...
        async openLog() {
            ...
            try {
                ...
                if (res.ok) {
                    var data = await res.json();
                    this.activityLogs = Array.isArray(data) ? data : [];
                } else this.activityLogs = [];
            } catch (e) { this.activityLogs = []; }
        },
        async clearLogs() {
            this.activityLogs = [];
            var ok = await this.pushFile('data/_logs.json', '[]', 'Admin: clear activity log');
            if (ok) this.flash('Log cleared.');
        },
        ...
        async saveBoardPlayer() {
            ...
            this.flash('Saved records for ' + player); this.startSyncNotify();
