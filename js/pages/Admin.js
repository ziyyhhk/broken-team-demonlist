export default Vue.defineAsyncComponent(async () => {
  ...
  var shMethods = [
    ...
    "          this.serverLevels = Array.isArray(data) ? data.map(function(l) { return Object.assign({}, l, { records: l.records || [], victor: l.victor || '', verifier: l.verifier || '' }); }) : [];",
    "        } else this.serverLevels = [];",
    "      } catch (e) { this.serverLevels = []; }",
    "    },",
    "    shAddRow() {",
    "      this.serverLevels.push({ id: '', name: '', author: '', victor: '', verifier: '', verification: '', thumbnail: '', length: '', note: '', tags: [], records: [] });",
    "    },",
    "    shMove(i, dir) {",
    "      var j = i + dir;",
    ...
    "      var payload = this.serverLevels.map(function(l) { return Object.assign({}, l); });",
  ...
  var newPanel = [
    ...
    '<h2>Server Hardest</h2>',
    '<p class="admin-hint">Creator = publisher. Victor = who beat it on the server. Verifier = optional. Thumbnail = direct image (i.imgur.com/xxx.png — not album links).</p>',
    ...
    '<input class="admin-input" v-model="lv.thumbnail" placeholder="Thumbnail direct image URL" style="flex:1;min-width:10rem" />',
  ...
  var boardPanel = [
    ...
    '<h2>Leaderboard</h2>',
    '<p class="admin-hint">Players come from level records + verifiers.</p>',
