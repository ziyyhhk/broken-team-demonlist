  ...
  <div class="submit-wrap" v-if="!loading">
    <div class="submit-main">
      <div class="submit-card">
        ...
        <div class="submit-field" style="margin-bottom:1rem">
          <label>Which list is this? <span class="req">*</span></label>
          <select v-model="form.listTarget">
            <option value="main">Main List</option>
            <option value="server_hardest">Server Hardest</option>
            <option value="impossible">Impossible List</option>
            <option value="platformer">Platformer List</option>
          </select>
        </div>
        ...
        <div class="submit-grid">
          ...
          <div class="submit-field" v-if="!isImpossible">
          ...
          <div class="submit-field" v-if="isImpossible">
            <label>Showcase link <span class="req">*</span></label>
            <input v-model="form.showcase" type="url" placeholder="Showcase / preview link only" />
          </div>
          ...
          <div class="submit-field submit-field--full" v-if="!isImpossible">
      ...
      <div class="submit-prev">
        ...
        <ul class="submit-prev-list" v-else>
          <li v-for="s in pagedPrev" :key="s.id">
            ...
            <div class="submit-prev-links" v-if="s.link || s.rawFootage || s.showcase">
              <a v-if="linkHref(s.link || s.showcase)" :href="linkHref(s.link || s.showcase)" target="_blank" rel="noopener">{{ s.listTarget === 'impossible' ? 'Showcase' : 'Watch completion' }}</a>
              <span v-else-if="s.link || s.showcase">{{ s.link || s.showcase }}</span>
      ...
      <div class="submit-rules-card">
        ...
        <div class="submit-rules-block">
          ...
          <ul class="submit-rules-list">
            ...
            <li>Previous attempt + full death before clear.</li>
...
`,
  methods: {
    ...
    validate() {
      ...
      if (!this.form.listTarget) return 'Select which list this is for.';
      ...
      if (this.isImpossible) {
        if (!(this.form.showcase || '').trim()) return 'Showcase link is required.';
      } else {
      ...
      if (!this.isImpossible && (!Number.isFinite(pct) || pct < 1 || pct > 100)) {
    ...
    async onSubmit() {
      ...
      try {
        const entry = {
          ...
          status: 'pending',
          mode: this.mode,
          listTarget: this.form.listTarget,
          player: (this.form.player || '').trim(),
          discordUser: (this.form.discordUser || '').trim(),
          displayName: (this.form.displayName || '').trim(),
          levelPath: this.form.levelPath,
          levelName: this.resolveLevelName(),
          percent: this.isImpossible ? 100 : Number(this.form.percent) || 100,
          link: (this.form.link || '').trim(),
          device: (this.form.device || '').trim(),
          modMenu: (this.form.modMenu || '').trim(),
          customId: (this.form.customId || '').trim(),
          creator: (this.form.creator || '').trim(),
          verifier: (this.form.verifier || '').trim(),
          showcase: (this.form.showcase || '').trim(),
          length: (this.form.length || '').trim(),
          attempts: this.form.attempts === '' || this.form.attempts == null ? '' : String(this.form.attempts),
          rawFootage: (this.form.rawFootage || '').trim(),
          notes: (this.form.notes || '').trim(),
        };

        if (this.isImpossible) {
          entry.link = entry.showcase;
          entry.device = '';
          entry.modMenu = '';
          entry.percent = 100;
          entry.attempts = '';
